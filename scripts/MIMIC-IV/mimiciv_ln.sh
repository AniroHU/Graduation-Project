#!/bin/bash
# ==============================================================================
# MIMIC-IV 注意力架构对比 (LayerNorm) — GraFITi 协议
# 102 变量, 48h ICU, 随机 70/20/10 划分
# 观测/预测: 24→12, 24→24, 36→6, 36→12 (GraFITi Table 3)
# ==============================================================================

root_path_name=./dataset/
data_path_name=MIMICIV
model_id_name=MIMICIV
data_name=MIMICIV
...
do
    ...
    if [ "$model_name" = "Encoder" ]; then
        cur_e_layers=6; cur_d_layers=0
    elif [ "$model_name" = "Decoder" ] || [ "$model_name" = "Prefix_decoder" ]; then
        cur_e_layers=0; cur_d_layers=6
    else
        cur_e_layers=$e_layers; cur_d_layers=$d_layers
    fi
    ...
    python -u run_longExp.py \
      ...
      --run_train --run_test \
      --norm layer --time_aware --time_embed_type continuous --freq h
