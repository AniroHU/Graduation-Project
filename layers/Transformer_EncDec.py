...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, missing_mask=None):
        # x [B, L, D]
    ...
    def forward(self, x, attn_mask=None, missing_mask=None):
        ...
        if self.conv_layers is not None:
            for attn_layer, conv_layer in zip(self.attn_layers, self.conv_layers):
                x, attn = attn_layer(x, attn_mask=attn_mask, missing_mask=missing_mask)  # 经过attn_layer后，x的维度不变，仍然为[B,L,D]，也即[32, 96, 512]（但是随着层数向上，96这一维每过一层就会除2；会不断变成48，24，12……）
            ...
            for attn_layer in self.attn_layers:
                x, attn = attn_layer(x, attn_mask=attn_mask, missing_mask=missing_mask)
                attns.append(attn)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, missing_mask=None, cross_missing_mask=None):
        # 1. Masked Self-Attention (decoder自己的自注意力)
        ...
        # 2. Cross-Attention (queries来自decoder, keys/values来自encoder)
        # cross_missing_mask 用于屏蔽 encoder 中的缺失/padding key
        x = x + self.dropout(self.cross_attention(
            x, cross, cross,
            attn_mask=cross_mask,
            missing_mask=cross_missing_mask
        )[0])
        # 第二次的add & norm
        y = x = self.norm2(x)
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, missing_mask=None, cross_missing_mask=None):
        for layer in self.layers:
            x = layer(x, cross, x_mask=x_mask, cross_mask=cross_mask,
                      missing_mask=missing_mask, cross_missing_mask=cross_missing_mask)
...
class DecoderLayer_wo_CrossAttn(nn.Module):
    ...
    def forward(self, x, x_mask=None, cross_mask=None, missing_mask=None):
...
class Decoder_wo_CrossAttn(nn.Module):
    ...
    def forward(self, x, x_mask=None, cross_mask=None, missing_mask=None):
        for layer in self.layers:
            x = layer(x, x_mask=x_mask, cross_mask=cross_mask, missing_mask=missing_mask)
