...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask_x is not None:
            _mm = missing_mask_x.permute(0, 2, 1)
            ...
            enc_missing = _mm.reshape(_bs * _nv, _pn)
        enc_out, attns = self.encoder1(enc_out, attn_mask=enc_self_mask, missing_mask=enc_missing)  # [(bs*channel) x input_patch_num x d_model]
        ...
        dec_out = self.encoder2(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, missing_mask=dec_missing, cross_missing_mask=enc_missing)  # [(bs*channel) x output_patch_num x patch_len]
