...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc,
        ...
        # 将 timestamps 从 [B, seq_len+pred_len] 适配到 [B*nvars, total_patch_num]
        # 这些 Masked Encoder 模型的 timestamps 必须覆盖 input+output 的完整范围
        if timestamps is not None:
            # timestamps: [B, seq_len+pred_len] or [B, seq_len]
            # 如果只有 seq_len 长度，需要生成 output 部分的虚拟时间戳
            if timestamps.shape[1] == self.seq_len:
                # 为 pred_len 部分生成延续的时间戳
                last_ts = timestamps[:, -1:]
                interval = (timestamps[:, -1:] - timestamps[:, -2:-1]).clamp(min=0.01)
                future_ts = last_ts + interval * torch.arange(1, self.pred_len + 1, device=timestamps.device).unsqueeze(0)
                timestamps_full = torch.cat([timestamps, future_ts], dim=1)
            else:
                timestamps_full = timestamps
            _ts = timestamps_full.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)  # [B, total_patch_num]
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            _ts_enc = _ts.reshape(bs * nvars, -1)
        else:
            _ts_enc = None
        enc_out = self.enc_embedding(x_enc, mask_token=mask_token, timestamps=_ts_enc)  # [(bs*channel) x total_patch_num x d_model]
        # 将embedding后的数据输入到encoder中
        # 生成 missing_mask (patch 级)
        enc_missing = None
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask is not None:
            # missing_mask: [B, seq_len, C] → 只覆盖 input 部分
            _mm = missing_mask.permute(0, 2, 1)  # [B, C, seq_len]
            _mm = _mm.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _mm = _mm.sum(dim=-1).float() / self.patch_len  # observation density [0,1]  # [B, C, input_patch_num]
            # output_patch 部分 mask=0 (全是 mask_token，没有真实观测)
            _mm_out = torch.zeros(_mm.shape[0], _mm.shape[1], self.output_patch_num, device=_mm.device)
            _mm = torch.cat([_mm, _mm_out], dim=-1)  # [B, C, total_patch_num]
            _bs, _nv, _pn = _mm.shape
            enc_missing = _mm.reshape(_bs * _nv, _pn)
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask, missing_mask=enc_missing)  # [(bs*channel) x total_patch_num x d_model]
