...
class Model(nn.Module):
    ...
    def forward(self, x_dec, x_mark_dec,
        ...
        # 将 timestamps 从 [B, L] 适配到 [B*nvars, patch_num]
        ...
        dec_out = self.decoder(dec_out, x_mask=self.dec_mask, missing_mask=dec_missing)  # [(bs*channel) x output_patch_num x patch_len]
    ...
    #     # 生成decoder缺失掩码
        ...
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask is not None:
            if missing_mask.dim() == 2:
                # 已经是 [B, L] 行级掩码 (从 exp_main 传入的 decoder 拼接掩码)
                _mm = missing_mask.unsqueeze(1).expand(-1, nvars, -1)  # [B, C, L]
            else:
                _mm = missing_mask.permute(0, 2, 1)  # [B, C, L]
            _mm = _mm.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _mm = _mm.sum(dim=-1).float() / self.patch_len  # observation density [0,1]
            _bs, _nv, _pn = _mm.shape
            dec_missing = _mm.reshape(_bs * _nv, _pn)
        
        dec_out = self.decoder(dec_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, missing_mask=dec_missing)  # [(bs*channel) x output_patch_num x patch_len]
