...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Time-Aware 连续时间嵌入配置
        time_aware = getattr(configs, 'time_aware', False)
        time_embed_type = getattr(configs, 'time_embed_type', 'continuous')
        ...
        if configs.embed_type == 0:
            ...
            self.enc_embedding = DataEmbedding(self.patch_len, configs.d_model, configs.embed, configs.freq,
                                            configs.dropout, time_aware=time_aware, time_embed_type=time_embed_type)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
                ...
                timestamps_x=None, timestamps_y=None,
                missing_mask_x=None, missing_mask_y=None):
        ...
        # 将 timestamps 从 [B, seq_len] 适配到 [B*nvars, patch_num]
        # 每个 patch 取该 patch 内时间戳的均值作为该 patch 的连续时间
        if timestamps_x is not None:
            # timestamps_x: [B, seq_len] → [B, patch_num, patch_len] → mean → [B, patch_num]
            _ts = timestamps_x.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # [B, patch_num, patch_len]
            _ts = _ts.mean(dim=-1)  # [B, patch_num]
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)  # [B, nvars, patch_num]
            _ts_enc = _ts.reshape(bs * nvars, -1)  # [B*nvars, patch_num]
        else:
            _ts_enc = None
        enc_out = self.enc_embedding(x_enc, timestamps=_ts_enc)  # [(bs*channel) x input_patch_num x d_model]
        # 将embedding后的数据输入到encoder中
        # 生成时间步级缺失掩码 (用于注意力层)
        # missing_mask: [B, L, C] → row_mask: [B*nvars, patch_num] (per-channel)
        # 由于 channel-independence，每个 channel 独立处理
        enc_missing = None
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask_x is not None:
            # missing_mask: [B, seq_len, C] → 转为 per-channel mask
            # 取每个变量自己的 mask: [B, seq_len, C] → permute → [B, C, seq_len]
            _mm = missing_mask_x.permute(0, 2, 1)  # [B, C, seq_len]
            # unfold 到 patch 级: [B, C, patch_num, patch_len]
            _mm = _mm.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            # 每个 patch 内有任一观测则该 patch 有效: [B, C, patch_num]
            _mm = _mm.sum(dim=-1).float() / self.patch_len  # observation density [0,1]
            _bs, _nv, _pn = _mm.shape
            enc_missing = _mm.reshape(_bs * _nv, _pn)  # [B*C, patch_num]
        
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask, missing_mask=enc_missing)  # [(bs*channel) x input_patch_num x d_model]

        # 对于decoder，我们同样需要做一次embedding
        # ! 同理，这里也不需要x_mark_dec信息了！
        # 将 timestamps_y 适配到 [B*nvars, patch_num]
        if timestamps_y is not None:
            _ts = timestamps_y[:, -self.pred_len:].unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            _ts_dec = _ts.reshape(bs * nvars, -1)
        else:
            _ts_dec = None
        dec_out = self.dec_embedding(x_dec, timestamps=_ts_dec)  # [(bs*channel) x output_patch_num x d_model]
        # 之后embedding后的数据和上面encoder的结果共同送入decoder中
        dec_missing = None
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask_y is not None:
            _mm = missing_mask_y[:, -self.pred_len:, :].permute(0, 2, 1)
            _mm = _mm.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _mm = _mm.sum(dim=-1).float() / self.patch_len  # observation density [0,1]
            _bs, _nv, _pn = _mm.shape
            dec_missing = _mm.reshape(_bs * _nv, _pn)
        dec_out = self.decoder(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, missing_mask=dec_missing, cross_missing_mask=enc_missing)  # [(bs*channel) x output_patch_num x patch_len]
