...
class Model(nn.Module):
    ...
    def forward(self, x_dec, x_mark_dec,
        ...
        dec_out = self.decoder(dec_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, missing_mask=dec_missing)
    ...
    def inference(self, x_dec, x_mark_dec,
        ...
        # 对于decoder，我们同样需要做一次embedding
        # ! 同理，这里也不需要x_mark_dec信息了！
        # timestamps reshape for inference (growing sequence)
        if timestamps is not None:
            _ts = timestamps.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            ...
            _ts_dec = None
        dec_out = self.dec_embedding(x_dec, timestamps=_ts_dec)
        # inference 阶段: 输入逐步增长，前面 seq_len 部分可能有缺失
        dec_missing = None
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask is not None:
            if missing_mask.dim() == 2:
                _mm = missing_mask.unsqueeze(1).expand(-1, nvars, -1)
