...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # timestamps reshape for enc
        if timestamps_x is not None:
            _ts = timestamps_x.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            _ts_enc = _ts.reshape(bs * nvars, -1)
        else:
            _ts_enc = None
        enc_out = self.enc_embedding(x_enc, timestamps=_ts_enc)
        # 生成 encoder missing_mask
        enc_missing = None
        ...
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask, missing_mask=enc_missing)

        # timestamps reshape for dec (x_dec includes start patch + pred)
        if timestamps_y is not None:
            _ts = timestamps_y.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            _ts_dec = _ts.reshape(bs * nvars, -1)
        else:
            _ts_dec = None
        dec_out = self.dec_embedding(x_dec, timestamps=_ts_dec)
        # autoregressive forward (teacher forcing): decoder 输入都是已知值，无需 missing_mask
        # cross_missing_mask 仍然需要，用于屏蔽 encoder 中的缺失位置
        dec_out = self.decoder(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, cross_missing_mask=enc_missing)
    ...
    def encode(self, x_enc, x_mark_enc,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None,
                timestamps=None, missing_mask=None):
        # 假设batch_size=32, seq_len=96, label_len=24, pred_len=48, channel=12, mark_channel=4
        # x_enc为[32, 96, 12], x_mark_enc为[32, 96, 4], x_dec为[32, 72, 12], x_mark_dec为[32, 72, 4]
        # 也即输入维度为[batch_size, seq_len, channel]
        ...
        # 先对encoder的输入数据做一次embedding
        # ! 注意，这里不需要x_mark_enc信息了！其中的位置编码足以标志出信息！
        # timestamps reshape for encode
        if timestamps is not None:
            _ts = timestamps.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            _ts_enc = _ts.reshape(bs * nvars, -1)
        else:
        ...
        # 生成 encoder missing_mask
        enc_missing = None
        if hasattr(self, 'time_aware') and self.time_aware and missing_mask is not None:
            _mm = missing_mask.permute(0, 2, 1)
            _mm = _mm.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _mm = _mm.sum(dim=-1).float() / self.patch_len  # observation density [0,1]
            _bs, _nv, _pn = _mm.shape
            enc_missing = _mm.reshape(_bs * _nv, _pn)
    ...
    def decode(self, x_dec, x_mark_dec, enc_out, enc_attns,
        ...
        # timestamps reshape for decode (growing sequence)
        if timestamps is not None:
            _ts = timestamps.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            _ts = _ts.mean(dim=-1)
            _ts = _ts.unsqueeze(1).expand(-1, nvars, -1)
            ...
            _ts_dec = None
        dec_out = self.dec_embedding(x_dec, timestamps=_ts_dec)
        # decode 阶段不生成 missing_mask (自回归生成的 patch 都是有效的)
