...
def _unpack_batch(batch, is_physionet=False):
    """
    统一解包 DataLoader 的输出。
    - 传统数据集返回 4 元素: (batch_x, batch_y, batch_x_mark, batch_y_mark)
    - Physionet2012 返回 8 元素: + timestamps + missing_masks
    
    Returns:
        batch_x, batch_y, batch_x_mark, batch_y_mark,
        timestamps_x, timestamps_y, missing_mask_x, missing_mask_y
    """
    if is_physionet and len(batch) == 8:
        return batch  # 直接返回全部 8 个元素
    elif is_physionet and len(batch) == 6:
        # 向后兼容: 没有 missing_mask 的旧版本
        batch_x, batch_y, batch_x_mark, batch_y_mark, timestamps_x, timestamps_y = batch
        return batch_x, batch_y, batch_x_mark, batch_y_mark, timestamps_x, timestamps_y, None, None
    else:
        batch_x, batch_y, batch_x_mark, batch_y_mark = batch[:4]
        return batch_x, batch_y, batch_x_mark, batch_y_mark, None, None, None, None
...
class Exp_Main(Exp_Basic):
    def __init__(self, args):
        ...
        # 判断是否是 Physionet2012 数据集
        self.is_physionet = (getattr(self.args, 'data', '') in ['Physionet2012', 'MIMICIV', 'MIMICIII'])
        # 判断是否使用 time-aware 连续时间嵌入
        self.time_aware = getattr(self.args, 'time_aware', False)
    ...
    def calc_corr_dmodel(self):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(train_loader):
                batch_x, batch_y, batch_x_mark, batch_y_mark, timestamps_x, timestamps_y, missing_mask_x, missing_mask_y = _unpack_batch(batch_data, self.is_physionet)
                ...
                if 'MoE' in self.args.model:
                    ...
                    outputs, mid_embedding = self.model(batch_x, batch_x_mark, timestamps=timestamps_x, missing_mask=missing_mask_x, return_mid_embedding=True)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, timestamps_x=timestamps_x, timestamps_y=timestamps_y, missing_mask_x=missing_mask_x, missing_mask_y=missing_mask_y)[0]
                    else:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, timestamps_x=timestamps_x, timestamps_y=timestamps_y, missing_mask_x=missing_mask_x, missing_mask_y=missing_mask_y)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(vali_loader):
                ...
                # 连续时间戳
                if timestamps_x is not None:
                    timestamps_x = timestamps_x.float().to(self.device)
                if timestamps_y is not None:
                    timestamps_y = timestamps_y.float().to(self.device)
                # 缺失掩码
                if missing_mask_x is not None:
                    missing_mask_x = missing_mask_x.float().to(self.device)
                if missing_mask_y is not None:
                    missing_mask_y = missing_mask_y.float().to(self.device)
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        elif 'Masked_encoder' in self.args.model or 'Encoder_zeros' in self.args.model or 'Encoder_overall' in self.args.model:
                            if timestamps_x is not None and timestamps_y is not None:
                                _ts_full = torch.cat([timestamps_x, timestamps_y[:, -self.args.pred_len:]], dim=-1)
                            else:
                                _ts_full = timestamps_x
                            outputs = self.model(batch_x, batch_x_mark, timestamps=_ts_full, missing_mask=missing_mask_x)
                        elif 'Encoder' in self.args.model:
                            outputs = self.model(batch_x, batch_x_mark, timestamps=timestamps_x, missing_mask=missing_mask_x)
                        else:
                            if self.args.output_attention:
                                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, timestamps_x=timestamps_x, timestamps_y=timestamps_y, missing_mask_x=missing_mask_x, missing_mask_y=missing_mask_y)[0]
                            else:
                    ...
                    if 'autoregressive' in self.args.model:
                        if 'Transformer' in self.args.model:
                            ...
                            # 先得到encoder的输出
                            enc_out, enc_attns = self.model.encode(batch_x, batch_x_mark, timestamps=timestamps_x, missing_mask=missing_mask_x)
                            
                            ys = start_symbol
                            # 构建自回归时间戳: 从 x 的最后一个 patch 开始
                            if timestamps_x is not None and timestamps_y is not None:
                                # start: 最后 patch_len 个时间戳
                                ar_ts = timestamps_x[:, -self.args.patch_len:]
                                ts_interval = (timestamps_x[:, -1:] - timestamps_x[:, -2:-1]).clamp(min=0.01)
                            else:
                                ar_ts = None
                            ...
                            for k in range(output_patch_num):
                                ys_len = ys.shape[1]
                                dec_out = self.model.decode(ys, batch_y_mark[:, :ys_len, :], enc_out, enc_attns, timestamps=ar_ts)
                                dec_out = dec_out[:, -self.args.patch_len:, :]
                                ys = torch.cat([ys, dec_out], dim=1)
                                # 扩展时间戳
                                if ar_ts is not None:
                                    new_ts = ar_ts[:, -1:] + ts_interval * torch.arange(1, self.args.patch_len + 1, device=ar_ts.device).unsqueeze(0)
                                    ar_ts = torch.cat([ar_ts, new_ts], dim=1)
                            ...
                            ys = start_symbol
                            # 构建自回归时间戳: 从 timestamps_x 开始，每步扩展
                            if timestamps_x is not None:
                                ar_ts = timestamps_x.clone()
                                ts_interval = (timestamps_x[:, -1:] - timestamps_x[:, -2:-1]).clamp(min=0.01)
                            else:
                                ar_ts = None
                            ...
                            for k in range(output_patch_num):
                                ys_len = ys.shape[1]
                                dec_out = self.model.inference(ys, batch_y_mark[:, :ys_len-self.args.seq_len, :], timestamps=ar_ts)
                                dec_out = dec_out[:, -self.args.patch_len:, :]
                                ys = torch.cat([ys, dec_out], dim=1)
                                # 扩展时间戳
                                if ar_ts is not None:
                    ...
                    # direct-output的decoder
                        ...
                        # 构建decoder的连续时间戳: 拼接 encoder 和 decoder 的时间
                        if timestamps_x is not None and timestamps_y is not None:
                            timestamps_dec = torch.cat([timestamps_x, timestamps_y[:, -self.args.pred_len:]], dim=-1)
                        else:
                            timestamps_dec = None
                        # 构建decoder的缺失掩码: 与 timestamps_dec 对应拼接
                        if missing_mask_x is not None and missing_mask_y is not None:
                            # 对每个变量取行级掩码 (任意变量有观测则该时间步有效)
                            # missing_mask: [B, L, C] -> row_mask: [B, L] (任一变量有观测即为1)
                            row_mask_x = (missing_mask_x.sum(dim=-1) > 0).float()
                            row_mask_y = (missing_mask_y[:, -self.args.pred_len:].sum(dim=-1) > 0).float()
                            missing_mask_dec = torch.cat([row_mask_x, row_mask_y], dim=-1)
                        else:
                            missing_mask_dec = None
                        outputs = self.model(dec_inp, batch_y_mark, timestamps=timestamps_dec, missing_mask=missing_mask_dec)
                        ...
                        or 'Encoder_zeros' in self.args.model \
                        or 'Encoder_overall' in self.args.model:
                        # Masked Encoder 需要 seq_len+pred_len 长度的 timestamps
                        if timestamps_x is not None and timestamps_y is not None:
                            _ts_full = torch.cat([timestamps_x, timestamps_y[:, -self.args.pred_len:]], dim=-1)
                        else:
                            _ts_full = timestamps_x
                ...
                # 验证 loss: 不规则时序只在真实观测位置计算
                if self.is_physionet and missing_mask_y is not None:
                    pred_mask = missing_mask_y[:, -self.args.pred_len:, f_dim:].detach().cpu()
                    if pred_mask.sum() > 0:
                        loss = (((pred - true) ** 2) * pred_mask).sum() / pred_mask.sum()
                    else:
    ...
    def train(self, setting, acf_lst=None):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                # 将连续时间戳也移到设备上 (Physionet2012 time-aware)
                if timestamps_x is not None:
                    timestamps_x = timestamps_x.float().to(self.device)
                if timestamps_y is not None:
                    timestamps_y = timestamps_y.float().to(self.device)
                ...
                if self.args.use_amp:
                    ...
                    # 计算loss
                    # 不规则时序: 只在真实观测位置计算 loss，忽略 ffill/bfill 填充位置
                    if self.is_physionet and missing_mask_y is not None:
                        pred_mask = missing_mask_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                        if pred_mask.sum() > 0:
                            loss = (((outputs - batch_y) ** 2) * pred_mask).sum() / pred_mask.sum()
                        else:
                            loss = criterion(outputs, batch_y)
                    else:
                        loss = criterion(outputs, batch_y)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    ...
                    if 'autoregressive' in self.args.model:
                        if 'Transformer' in self.args.model:
                            ...
                            for k in range(output_patch_num):
                                ys_len = ys.shape[1]
                                dec_out = self.model.inference(ys, batch_y_mark[:, :ys_len, :], timestamps=ar_ts)
                                dec_out = dec_out[:, -self.args.patch_len:, :]
                                ys = torch.cat([ys, dec_out], dim=1)
                                if ar_ts is not None:
                                    new_ts = ar_ts[:, -1:] + ts_interval * torch.arange(1, self.args.patch_len + 1, device=ar_ts.device).unsqueeze(0)
                ...
                # 收集缺失掩码
                if missing_mask_y is not None:
                    masks.append(missing_mask_y[:, -self.args.pred_len:, f_dim:].detach().cpu().numpy())
                else:
                    masks.append(np.ones_like(pred))
        ...
        # 评估指标
        if self.is_physionet:
            # 不规则时序: 在标准化空间上、只在真实观测位置评估
            # (与 GraFITi / t-PatchGNN 等 IMTS 文献一致)
            valid_idx = (masks == 1)
            if valid_idx.sum() > 0:
                preds_eval = preds[valid_idx].reshape(-1, 1)
                trues_eval = trues[valid_idx].reshape(-1, 1)
            else:
                preds_eval = preds
                trues_eval = trues
            
            # ====== 防爆盾 (Outlier Clipping) ======
            # MIMIC-IV 原始数据含有护士录入错误的极端异常值 (如心率 8500)
            # 标准化后仍可能残留 |z| > 10 的极端值，MSE 平方后会毁灭整个指标
            # 将 trues 和 preds 截断在 [-10, 10] 的安全范围内
            if 'MIMIC' in self.args.data or 'mimic' in getattr(self.args, 'data_path', '').lower():
                clip_threshold = 10.0
                n_extreme_true = (np.abs(trues_eval) > clip_threshold).sum()
                n_extreme_pred = (np.abs(preds_eval) > clip_threshold).sum()
                if n_extreme_true > 0:
                    print(f"⚠️ Clipping {n_extreme_true} extreme true values (max={np.max(np.abs(trues_eval)):.2f})")
                if n_extreme_pred > 0:
                    print(f"⚠️ Clipping {n_extreme_pred} extreme pred values (max={np.max(np.abs(preds_eval)):.2f})")
                preds_eval = np.clip(preds_eval, -clip_threshold, clip_threshold)
                trues_eval = np.clip(trues_eval, -clip_threshold, clip_threshold)
            # ==========================================
            
            mae, mse, rmse, mape, mspe, rse, corr = metric(preds_eval, trues_eval)
            print('mse:{}, mae:{} (observed-only, standardized)'.format(mse, mae))
            # 全量指标供参考
            mae_all, mse_all, _, _, _, _, _ = metric(preds, trues)
            print('  [all-positions] mse:{}, mae:{}'.format(mse_all, mae_all))
        else:
            mae, mse, rmse, mape, mspe, rse, corr = metric(preds, trues)
            print('mse:{}, mae:{}, rse:{}'.format(mse, mae, rse))
        
        f = open("result_p12.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{}'.format(mse, mae))
        if self.is_physionet:
            f.write('  |  all_mse:{}, all_mae:{}'.format(mse_all, mae_all))
        f.write('\n')
        f.write('\n')
        f.close()
        ...
        for cur_pred_len in self.args.multiple_pred_len_list:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(test_loader):
                    ...
                    trues.append(true)
                    if missing_mask_y is not None:
                        masks.append(missing_mask_y[:, -cur_pred_len:, f_dim:].detach().cpu().numpy())
                    else:
                        masks.append(np.ones_like(pred))
                    inputx.append(batch_x.detach().cpu().numpy())
                    # if i % 20 == 0:
